...
function LateReturns() {
  ...
  const fetchReturns = async () => {
    try {
      ...
      const res = await axios.get(
        "https://costume-rental-system.onrender.com/api/bookings/late-returns",
      );
  ...
  return (
    <div className="min-h-screen pt-20 pb-20" style={{ backgroundColor: '#fdf8e6' }}>
      ...
      <div className="fixed top-20 left-20 w-96 h-96 rounded-full blur-3xl pointer-events-none -z-10" style={{ backgroundColor: 'rgba(239, 71, 111, 0.3)' }} />
      <div className="fixed bottom-20 right-20 w-96 h-96 rounded-full blur-3xl pointer-events-none -z-10" style={{ backgroundColor: 'rgba(255, 209, 102, 0.2)' }} />
      ...
      <div className="max-w-6xl mx-auto px-4 relative z-10">
        ...
        {loading ? (
          <div className="bg-white border-4 border-black rounded-3xl p-12 text-center shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            ...
            <p className="text-2xl font-black text-black" style={{ fontFamily: "'Chewy', cursive" }}>Loading Late Returns...</p>
          </div>
        ) : bookings.length === 0 ? (
          <div className="bg-white border-4 border-black rounded-3xl p-12 text-center shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <p className="text-2xl font-black text-black" style={{ fontFamily: "'Chewy', cursive" }}>✨ No late returns</p>
            <p className="text-black font-bold mt-2">Everything's on track!</p>
          </div>
        ) : (
          <div className="bg-white border-4 border-black rounded-3xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr style={{ backgroundColor: '#ef476f' }}>
              ...
              <tbody>
                {bookings.map((b) => (
                  <tr key={b.id} className="border-b-2 border-black hover:bg-red-50">
